/**
 * \file color_table.c
 * \brief Definition of the color table structure and implementation of color table functions for image quantization.
 * \author Johann LY
 * \date January 2024
 *
 * This file defines color_table data structure and function to create, mange and manipulate color table.
 *
 */
#include <stdio.h>
#include <assert.h>
...
/**
 * \struct color_table
 * \brief Structure representing a color table for image quantization.
 */
struct color_table {
    /**
     * \brief Array of unique colors in the table.
     *
     * This dynamically allocated array stores the distinct colors extracted 
     * from an image.
     */
    color* suite_color;

    /**
     * \brief Number of colors in the table.
     *
     */
    int nb_color;

    /**
     * \brief Flag for memory management.
     *
     * - 1 (true): The table owns the `suite_color` array and is responsible for freeing it.
     * - 0 (false): The table is a reference to another table’s colors and should not free `suite_color`.
     */
    boolean owner;
};
